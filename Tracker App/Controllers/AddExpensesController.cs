...
namespace Tracker_App.Controllers
{
    public class AddExpensesController : Controller
    {
        ...
        public ActionResult GetProgressData()
        {
            ...
            var progressData = db.AddExpenses
             .Where(e => e.userEmail == name)
             .Include(e => e.expenseCategory)
             .GroupBy(e => e.expenseCategory.categoryName)
             .Select(g => new
             {
                 expenseAmount = g.Sum(e => e.expenseAmount),
                 categoryName = g.Key
             })
             .ToList();
        ...
        public ActionResult ProgressData()
        {

            string name = HttpContext.User.Identity.Name;
            ViewBag.count = db.AddExpenses
                .Include(g => g.expenseCategory)
                .Where(e => e.userEmail == name)
                .Select(g => g.expenseCategory.categoryName)
